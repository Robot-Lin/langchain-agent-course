...
这是 Stage 5 的课程入口。这一章的核心目标是：把 Agent 原型推进成工程系统，学会读懂连接层、知识层、数据层和评测层代码。

## 学完这一章之后应该做到什么

- 覆盖并理解这一章的核心能力：MCP、RAG、memory、database、API、observability、evaluation、governance
...
- 重点看懂工具接入层、检索链路、存储层、API 路由、评测钩子、权限和治理检查。
...
- 用户应能解释哪段代码负责检索、哪段负责持久化、哪段负责观测和评测。
